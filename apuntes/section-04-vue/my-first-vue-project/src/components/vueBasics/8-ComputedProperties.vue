<template>
  ...
  <div>
    <h4>CLASS EXCERCISE</h4>
    <h4>All Watch List</h4>
    <div v-for="(watch, index) in myWatchlist" :key="index">
      <h1>{{ watch.title }}</h1>
      <h2>{{ watch.year }}</h2>
      <ul v-for="(dispositivo, index) in watch.whereToWatch" :key="index">
        <li>{{dispositivo}}</li>
      </ul>
    </div>
    <h4>Only Watch Netflix - watchOnNetflix</h4>
    <div v-for="(watch, index) in watchOnNetflix" :key="index">
      <h1>{{ watch.title }}</h1>
      <h2>{{ watch.year }}</h2>
      <ul v-for="(dispositivo, index) in watch.whereToWatch" :key="index">
        <li>{{dispositivo}}</li>
...
const watchOnNetflix = computed(() => {
  return myWatchlist.filter((watchNetflix) => watchNetflix.whereToWatch.includes("Netflix"));
});
