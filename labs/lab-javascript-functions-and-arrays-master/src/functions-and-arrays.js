...
function averageNumbers(numeros) {
  ...
  if (!numeros.length) {
    ...
    let total = numeros.reduce((total, num) => total + num);
    resultado = total / length;
  }
...
function averageWordLength(words) {
  ...
  if (!words.length) {
    ...
    let total;
    words.forEach((word) => {
      arrayString.push(word.length);
      total = arrayString.reduce((total, letter) => total + letter);
    });
...
function uniquifyArray(array) {
  ...
  if (!array.length) {
    ...
    tempArray.sort();
    let tempArrayClear = [];
    console.log("ordenado: " + typeof tempArray);
    console.log(tempArray);
    for (let i = 0; i < tempArray.length; i++) {
      if (tempArray[i + 1] != tempArray[i]) {
        tempArrayClear.push(tempArray[i]);
      }
    }
    for (let i = 0; i < array.length; i++) {
      if (array[i] === tempArrayClear[i]) {
        resultado.push(tempArrayClear[i]);
